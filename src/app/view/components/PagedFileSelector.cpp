    ...
    if (ImGui::BeginTable("FilesSelectorTable", 4, ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_Resizable | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY, outerSize)) {
        ...
        for (size_t i = startIndex; i < endIndex; ++i) {
            ...
            if (ImGui::Checkbox("##check", &isSelected)) {
                if (isSelected) selectedPaths_.insert(fileInfo.path);
                else selectedPaths_.erase(fileInfo.path);
            }
            ImGui::PopID();
            ...
            ImGui::TableNextColumn();
            std::string ext = (fileInfo.extension.length() > 0 && fileInfo.extension[0] == '.') ? fileInfo.extension.substr(1) : fileInfo.extension;
...
void PagedFileSelector::renderPaginationControls() {
    ...
    if (ImGui::InputText("##PageInputSelector", pageInputBuffer_, sizeof(pageInputBuffer_), ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_EnterReturnsTrue)) {
