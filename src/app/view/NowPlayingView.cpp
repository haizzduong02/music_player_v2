...
NowPlayingView::~NowPlayingView()
{
    ...
    if (pauseTexture_)
        glDeleteTextures(1, &pauseTexture_);
    if (nextTexture_)
        glDeleteTextures(1, &nextTexture_);
    if (prevTexture_)
        glDeleteTextures(1, &prevTexture_);
    if (heartFilledTexture_)
        glDeleteTextures(1, &heartFilledTexture_);
    if (heartOutlineTexture_)
...
void NowPlayingView::renderProgressBar()
{
    if (!state_) return;
...
void NowPlayingView::renderMetadata()
{
    if (!state_ || !state_->getCurrentTrack()) return;

    auto track = state_->getCurrentTrack();
    const auto &meta = track->getMetadata();

    float colWidth = ImGui::GetColumnWidth();
    float buttonsWidth = 60.0f; 
...
void NowPlayingView::renderAlbumArt()
{
}

void NowPlayingView::renderPlaybackControls()
{
    float buttonWidth = 32.0f;
    float buttonHeight = 32.0f;
    float loopBtnWidth = 60.0f;
    float loopBtnHeight = 32.0f;
    ...
    if (state_ && playlistManager_)
    {
        ...
        if (track)
        {
            ...
            if (favPlaylist)
            {
                ...
                if (ImGui::ImageButton("##fav_controls", (ImTextureID)(intptr_t)iconTex,
    ...
    if (controller_)
    {
        ...
        if (mode == RepeatMode::ALL)
        {
            ...
            color = ImVec4(0.0f, 0.7f, 0.6f, 1.0f);
        }
        ...
        {
            ImGui::SetTooltip("Current Mode: %s",
                              (mode == RepeatMode::ALL ? "ALL" : (mode == RepeatMode::ONE ? "ONE" : "OFF")));
        }
    }
...
void NowPlayingView::renderVolumeControl()
{
...
void NowPlayingView::onPlayClicked()
{
    if (controller_)
    {
        bool isPlaying = (state_ && state_->getStatus() == PlaybackStatus::PLAYING);
        if (isPlaying)
            controller_->pause();
        else if (state_ && state_->getStatus() == PlaybackStatus::PAUSED)
            controller_->resume();
        else if (state_)
            controller_->play(state_->getCurrentTrack());
    }
}
...
void NowPlayingView::onFavoriteClicked(bool currentlyFavorite)
{
    if (state_ && playlistManager_)
    {
        auto track = state_->getCurrentTrack();
