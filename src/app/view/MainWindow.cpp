...
void MainWindow::render()
{
    ...
    { // Begin MainWindow
        ...
        ImGui::BeginGroup();
        {
            ...
            ImGui::BeginChild("ArtPanel", ImVec2(artWidth, artHeight), false,
                              ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
            renderAlbumArt();
            ImGui::EndChild();
    ...
    if (currentScreen_ == Screen::PLAYLIST && playlistView_)
        playlistView_->renderPopups();
    if (currentScreen_ == Screen::HISTORY && historyView_)
        historyView_->renderPopups();
#endif
}
        ...
        if (currentTrackPath_ != track->getPath())
        {
            ...
            if (meta.hasAlbumArt && !meta.albumArtData.empty())
            {
                ...
                unsigned char *imageData =
                    stbi_load_from_memory(meta.albumArtData.data(), static_cast<int>(meta.albumArtData.size()), &width,
                                          &height, &channels, 4);
