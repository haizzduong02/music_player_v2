...
void PlaylistView::render()
{
    ...
    {
        if (playlists[i]->getName() == "Now Playing")
            continue;
        ...
        if (ImGui::Selectable(playlists[i]->getName().c_str(), isSelectedPl, 0,
                              ImVec2(sidebarWidth - delBtnWidth - 5, 0)))
        {
            ...
            if (playlistController_ && selectedPlaylist_)
            {
                ...
                adapter = std::make_shared<PlaylistTrackListController>(playlistController_, selectedPlaylist_,
                                                                        playbackController_);
...
void PlaylistView::renderAddSongsPopup()
{
    ...
    if (ImGui::BeginPopupModal("Add Songs to Playlist", &showAddSongsPopup_))
    {
        ...
        ImGui::BeginGroup();
        {
            ...
            if (ImGui::Button("Browse Files..."))
            {
                if (fileBrowserView_)
                {
                    ...
                    fileBrowserView_->setOnFilesAddedCallback(
        ...
        auto addTracks = [&](const std::vector<std::string> &paths)
        {
            ...
            if (count > 0)
                Logger::info("Added " + std::to_string(count) + " tracks.");
